<template>
  ...
  <!-- <querys :querys="querysArr" @changeOptions="changeOptions" /> -->
  <van-dropdown-menu>
    <van-dropdown-item
  ...
  <van-list v-model="loading" :finished="finished" :immediate-check="false" finished-text="没有更多了"
    @load="onLoad">
    <div class="item" v-for="(v, index) in data" :key="index" @click="handleTap(v)">
      <div class="item-name">
  ...
  <!-- 
      <div class="item-info" v-if="v == 2" @click="addItemInfo">
        <span>完善违法信息<van-icon name="arrow" color="#999" /></span>
      </div> -->
    </div>
  </van-list>
...
export default {
  ...
  methods: {
    ...
    handleTap(row) {
      ...
      let id = row.id;
      let isDone = row.isHandle === 'Y';
      this.$router.push({name: 'taskDetail', query: {id, isDone: isDone}});
    },
    // 切换选择条件
    changeOptions(v) {
       this.getTodoList(1);

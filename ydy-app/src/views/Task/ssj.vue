...
export default {
  ...
  methods: {
    ...
    getTodoList(v) {
      ...
      this.$http.post(url.getListByIdDoubleRandom, this.query).then(res => {
        toast.clear();
        this.loading = false;
        if (!res.list.length) {
          this.finished = true;
        ...
        this.$store.commit('setSelectRow', null);
      })
    },
    // 滚动加载分页请求
    onLoad () {
    ...
    handleTap(row) {
      ...
      let id = row.id;
      this.$router.push({name: 'taskDetail1', query: {id}});
    },
    // 切换选择条件
    changeOptions(v) {
       this.getTodoList(1);
